...
lib/I18n.ps1            多言語：日本語/英語の文字列・状態コード対訳（Get-MphLang / Get-MphI18n）
...
- **多言語（`lib/I18n.ps1`）**: OS の UI 言語が日本語なら日本語、それ以外は英語
  （`MPH_LANG=ja|en` で上書き可）。UI 文言（キャプション・ボタン・間隔ラベル等）も
  データ値（ol_stat/status/mode/join、WiiLink の type/joinable/role）も両言語対応。
  言語はビューワが判定して `$sync.Lang` で各ワーカーへ渡し、取得時に言語化する。
  ol_stat の `G`/`g`・`C`/`c` を区別するため、対訳表は **Ordinal 比較の case-sensitive
  ハッシュ**で構築している（PowerShell 既定の `@{}` は大小無視でキー衝突するため）。
